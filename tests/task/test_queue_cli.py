...
from xax.utils.launcher.queue_state import QueuedJob, edit_queue_state, read_queue_state
...
def test_queue_stop_removes_unit_and_resets_state(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    def _fake_cleanup_stale_processes(_out: object) -> None:
...
def test_collect_metric_points_skips_non_numeric_tensor_values(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    class _FakeAccumulator:
        def __init__(self, _path: str, size_guidance: dict[object, int]) -> None:
            assert size_guidance

        def Reload(self) -> None:  # noqa: N802
            return None

        def Tags(self) -> dict[str, list[str]]:  # noqa: N802
            return {"scalars": ["loss"], "tensors": ["status", "num"]}

        def Scalars(self, tag: str) -> list[SimpleNamespace]:  # noqa: N802
            if tag != "loss":
                return []
            return [SimpleNamespace(step=1, wall_time=1.0, value=2.5)]

        def Tensors(self, tag: str) -> list[SimpleNamespace]:  # noqa: N802
            if tag == "status":
                return [SimpleNamespace(step=2, wall_time=2.0, tensor_proto="status_proto")]
            if tag == "num":
                return [SimpleNamespace(step=3, wall_time=3.0, tensor_proto="num_proto")]
            return []

    def fake_make_ndarray(tensor_proto: object) -> np.ndarray:
        if tensor_proto == "status_proto":
            return np.array(["running"], dtype=object)
        if tensor_proto == "num_proto":
            return np.array([7.5], dtype=np.float32)
        raise AssertionError(f"unexpected tensor proto: {tensor_proto!r}")

    class _FakeEventAccumulatorModule:
        SCALARS = object()
        TENSORS = object()
        EventAccumulator = _FakeAccumulator

    class _FakeTensorUtilModule:
        @staticmethod
        def make_ndarray(tensor_proto: object) -> np.ndarray:
            return fake_make_ndarray(tensor_proto)

    monkeypatch.setattr("xax.cli.queue._event_subdirs", lambda _root: [("light", tmp_path)])
    monkeypatch.setattr(
        "xax.cli.queue._load_tensorboard_modules",
        lambda: (_FakeEventAccumulatorModule, _FakeTensorUtilModule),
    )

    points = _collect_metric_points(tmp_path)
    tags = [point.tag for point in points]
    assert "loss" in tags
    assert "num" in tags
...
def test_resolve_existing_run_dir_falls_back_under_runs_root(
    ...
    monkeypatch.setattr("xax.cli.queue.get_runs_dir", lambda: runs_root)
...
def test_resolve_existing_run_dir_falls_back_to_legacy_run_dir_env(
    ...
    stale_run_dir = logs_root / "queue_mnist_flow2" / "mnist_classification" / "run_000"
    monkeypatch.setattr("xax.cli.queue.get_runs_dir", lambda: None)
...
def test_status_formats_recent_jobs_as_table_and_shows_observer_gpus(
    ...
    monkeypatch.setattr("xax.cli.queue.read_queue_state", lambda: state)
    monkeypatch.setattr("xax.cli.queue.read_observer_info", lambda: observer_info)
    monkeypatch.setattr("xax.cli.queue.list_jobs", lambda: [job])
    monkeypatch.setattr("xax.cli.queue.is_observer_active", lambda **_kwargs: True)
...
def test_tail_defaults_to_running_job_when_job_id_is_omitted(
    ...
    monkeypatch.setattr("xax.cli.queue.get_running_job", lambda: running_job)
    monkeypatch.setattr("xax.cli.queue.get_job", lambda _job_id: None)
    ...
    assert system_exit.value.code == 0
    assert seen["path"] == Path("/tmp/job-0000002-task.log")
    assert seen["lines"] == 7
    assert seen["follow"] is True


def test_tail_without_job_id_errors_when_nothing_is_running(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr("xax.cli.queue.get_running_job", lambda: None)

    with pytest.raises(SystemExit) as system_exit:
        main(["tail"])

    assert system_exit.value.code == 1
    assert "No running queued job to tail" in capsys.readouterr().err


def test_run_queued_job_always_uses_multi_device_launcher(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    job = {
        ...
        "observer_log_path": str(tmp_path / "queue_observer.log"),
        "enqueued_at": 1.0,
        "started_at": None,
        "ended_at": None,
        "pid": None,
    ...
    monkeypatch.setattr("xax.cli.queue.get_job", lambda lookup_job_id: job if lookup_job_id == job_id else None)

    class _FakeRunnableMixin:
        pass

    class _FakeTask(_FakeRunnableMixin):
        pass

    class _FakeTaskBase:
        @classmethod
        def from_task_key(cls, _task_key: str) -> type[_FakeTask]:
            return _FakeTask

    monkeypatch.setattr(
        "xax.cli.queue._load_task_runtime_modules",
        lambda: (SimpleNamespace(BaseTask=_FakeTaskBase), SimpleNamespace(RunnableMixin=_FakeRunnableMixin)),
    )

    launch_calls: list[tuple[type, tuple[object, ...], bool | list[str]]] = []

    class _FakeLauncher:
        def launch(self, task: type, *cfgs: object, use_cli: bool | list[str] = True) -> None:
            launch_calls.append((task, cfgs, use_cli))

    def _fake_get_queue_launcher() -> _FakeLauncher:
        return _FakeLauncher()

    monkeypatch.setattr("xax.cli.queue._get_queue_launcher", _fake_get_queue_launcher)

    return_code = _run_queued_job(job_id)
    assert return_code == 0
    assert len(launch_calls) == 1
    task_type, launch_cfgs, use_cli = launch_calls[0]
    assert task_type is _FakeTask
    assert launch_cfgs == (config_path,)
