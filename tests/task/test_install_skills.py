...
def test_install_bundled_skills_copies_agents_tree(tmp_path: Path) -> None:
    ...
    assert (skill_root / "scripts" / "start_experiment_session.py").exists()
    assert (skill_root / "scripts" / "sync_queue_status.py").exists()
    assert (skill_root / "scripts" / "upsert_experiment_log.py").exists()
    assert (skill_root / "scripts" / "render_experiment_report.py").exists()
